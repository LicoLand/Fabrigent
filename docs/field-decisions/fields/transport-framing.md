# Field Review: Transport Framing Discriminator
...
| Decision ID | `FLD-transport-framing` |
...
| Authority targets | `spec/v1/transport/registry.json`, the HTTPS Transport schemas and grammar, and the stable-core Pairwise Protection transcript |
| Predecessor or successor | None |
| Current conclusion | The Transport Profile selects the carrier parser without a structured-body `contractVersion`; the authoritative Protocol Line and Protection Profile remain endpoint-authenticated. |
...
Separates carrier parser dispatch from Endpoint-authenticated protocol selection
...
| Implementation complexity | Duplicate protected and carrier-visible identifiers create mismatch rules and extra test cases. |
...
| Test | Finding |
| --- | --- |
| Required action | Select one exact carrier framing/parser and reject unknown input. |
...
| Duplicate-authority risk | High: a carrier-visible version and protected version can disagree or enable downgrade. |

## Value model

The authenticated Station descriptor's `transportProfileId` selects HTTPS
Transport v1. That Profile fixes HTTP/2 over TLS 1.3, operation-specific
methods and request targets, exact request and response media types, canonical
control-body CBOR, and raw protected-packet framing. No body-level
`contractVersion`, media-type parameter, negotiated selector, or fallback is
admitted. The authoritative Protocol Line and stable-core Protection Profile
are separately bound by Endpoint-authenticated establishment; unknown,
retired, downgrade-selected, or mismatched values fail closed.
...
carrier parser is selected by the Transport Profile, `contractVersion` is not a
...
HTTPS Transport v1 and the stable-core Pairwise Protection contract now close
the external parser selection, authenticated identities, precedence,
mismatch, downgrade, and retirement rules.

## Definition evidence

The definition status is `SPECIFIED`. The linked closed Transport and
Protection authorities define exact parser dispatch and forbid a structured
version field or negotiation fallback.
